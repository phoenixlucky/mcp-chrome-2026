...
<p align="center">
  ...
  基于 Model Context Protocol，向 AI 助手开放 59 个浏览器能力
</p>
...
> **`chrome_scroll` 真人滚动模式** — 模拟人类滚轮行为，专为动态页面与懒加载设计。
>
> - 🧑‍🦱 **真人滚动模式（`mode: 'human'`）** — 原生 wheel 输入逐像素滚动，可触发页面真实滚动监听器；分步推进 + 加减速曲线模拟真人节奏
> - ⏳ **`humanLazyLoad: true`** — 滚动到动态区域自动暂停，等待懒加载内容出现后继续
> - 📌 **`stickyHeaderSelector`** — 自动避让吸顶头部，避免滚动目标被遮挡
> - 🔧 工具描述与参数同步完善
> - 🔧 版本统一为 v1.8.1
...
| **🧠 语义搜索**<br/>向量数据库 + 本地嵌入<br/>跨标签页内容发现      | **⚡ SIMD 加速**<br/>WASM 优化引擎<br/>向量运算 4-8× 更快          | **📊 59 工具**<br/>导航 / 截图 / 表单<br/>书签 / 历史 / 网络 | **🔄 跨标签页操作**<br/>多标签 / 多窗口<br/>无缝协同管理      |
...
| ✅ 已实现                                           | 🎯 规划中                                         |
| --------------------------------------------------- | ------------------------------------------------- |
| **59 MCP 工具** — 浏览器全能力覆盖                  | **认证与权限管理** — API Key / OAuth 接入         |
