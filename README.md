...
> **元素标记全面升级** — 标记元素后可直接预览并复制定位代码，无需手动导出 JSON。
>
> - 🪟 **代码生成弹窗** — 标记元素后弹窗展示定位代码，支持一键复制
> - 🌐 **多语言代码生成** — JavaScript（querySelector / XPath）和 Python（Selenium By）
> - 📋 **智能复制** — Clipboard API + fallback 兼容，确保随处可用
> - ⌨️ **Escape 快捷关闭** — 弹出代码后按 Esc 即可关闭弹窗
