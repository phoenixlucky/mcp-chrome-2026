...
> **主题视觉升级 + 向量库/索引稳定性** — 观感与可靠性双提升。
>
> - 🎨 **全应用立绘背景** — Popup / Sidepanel / Welcome / Builder 卡片透明化并与背景图融合（`color-mix` 半透明材质、`center top` 构图修正）
> - 💾 **IDBFS 同步竞态修复** — 等待 sync 回调完成后继续，避免向量库同步竞态
> - 🧵 **内容索引按 tab 去重** — 同一标签页索引任务复用进行中 Promise，避免并发重复索引
> - 🗄️ **better-sqlite3 升级 ^13** — N-API 预编译二进制，安装免源码编译
> - 🔧 版本统一为 v2.4.10
