...
> **目标标签页解析重构 + ref 过期恢复** — 交互稳定性与工具可用性大幅提升。
>
> - 🎯 **目标标签页解析重构** — 显式 `tabId` 权威，目标关闭/缺失时明确报错而非静默回退到无关标签页；多工具支持 `windowId` 窗口选择
> - 🔧 **click/fill 目标恢复** — ref 过期时用保留的 CSS/XPath selector 重新解析目标（React 重建 DOM 节点后仍可命中）；点击/填充助手支持 XPath 选择器
> - 🌐 **读取类工具支持 `url` 导航** — 提供 `url` 时先导航再读取目标标签页（可选 `background` 后台打开）
