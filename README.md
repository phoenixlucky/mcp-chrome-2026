...
<p align="center">
  ...
  基于 Model Context Protocol，向 AI 助手开放 68 个浏览器能力
</p>
...
> **交互稳定性与健壮性增强**
>
> - 🛡️ **受限页面注入保护** — 在 `chrome://`、`edge://`、`devtools:` 等受限页面注入脚本前主动检测并给出清晰报错，不再抛出晦涩的 Chrome 拒绝错误
> - ⌨️ **键盘参数强校验** — `chrome_keyboard` 的 `keys` 参数运行时强制为非空字符串，拒绝数组/对象等非法输入
> - 🖱️ **点击助手增强** — 等待元素可见后再点击；不可见元素自动提升到可点击祖先元素
> - 📝 **填充助手增强** — 命中测试前处理屏幕外可渲染候选元素，填充更可靠
> - 🔧 版本统一为 v2.0.1
...
| **🧠 语义搜索**<br/>向量数据库 + 本地嵌入<br/>跨标签页内容发现      | **⚡ SIMD 加速**<br/>WASM 优化引擎<br/>向量运算 4-8× 更快          | **📊 68 工具**<br/>导航 / 截图 / 表单<br/>书签 / 历史 / 网络 | **🔄 跨标签页操作**<br/>多标签 / 多窗口<br/>无缝协同管理      |
...
| ✅ 已实现                                           | 🎯 规划中                                         |
| --------------------------------------------------- | ------------------------------------------------- |
| **68 MCP 工具** — 浏览器全能力覆盖                  | **认证与权限管理** — API Key / OAuth 接入         |
