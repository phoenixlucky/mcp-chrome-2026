...
const launchDates: Record<string, string> = {
  ...
  chrome_locate_element: '2026-08-13',
  ...
  chrome_select_all_items: '2026-08-13',
  ...
  chrome_proxy_rotate: '2026-08-04',
...
const zhToolParameterDescriptions: Record<string, string> = {
  ...
  'search_tabs_content.query': '要搜索的文本或主题。',
  'search_tabs_content.tabIds': '要搜索的 1 到 5 个标签页 ID。',
  'chrome_history.text': '在历史 URL 和标题中搜索的文本；留空则返回全部历史条目。',
  'chrome_scoped_action.text': '点击时可选的可见文本过滤条件。',
  'chrome_spa_fetch.tabId': '按 ID 指定现有标签页（默认：新建标签页）。',
  'chrome_extract.waitForSelector': '为 true 时，提取前等待选择器出现在 DOM 中（默认 true）。',
  'chrome_wait.needResponseBody': '网络事件：可用时返回匹配的响应正文。',
  'chrome_switch_tab.tabId': '要切换到的标签页 ID。',
  'chrome_network_capture.tabId': '仅采集此标签页；停止时只停止此标签页，不影响其他采集。',
  'chrome_close_tabs.url': '关闭匹配该 URL 的标签页；可替代 tabIds 使用。',
  'chrome_bookmark_add.url': '要收藏的 URL；未提供时使用当前激活标签页的 URL。',
  'chrome_bookmark_delete.url': '要删除的书签 URL；未提供 bookmarkId 时使用。',
  'chrome_bookmark_delete.title': '按 URL 删除时的书签标题，用于辅助匹配。',
  'chrome_task_context.url': '创建时的初始 URL。',
  'chrome_spa_fetch.url': '要抓取内容的 SPA 目标 URL。',
  'chrome_navigate.url': '要打开的 URL。特殊值 "back" 或 "forward" 用于在目标标签页中浏览历史。',
  'chrome_console.url': '要导航并采集控制台的 URL；未提供时使用当前激活标签页。',
  'chrome_proxy_diagnostics.action': 'status 只读取状态；test 还会验证代理出口 IP。',
  'chrome_handle_dialog.action': '接受 | 取消',
  'chrome_block_images.action': '"start" 阻止后续图片请求；"stop" 恢复图片加载。',
  'chrome_network_capture.action': '要执行的操作："start" 开始采集，"stop" 结束并返回结果。',
  'chrome_upload_file.selector': '文件输入元素（input[type="file"]）的 CSS 选择器。',
  'chrome_get_page_text.selector':
    '可选 CSS 选择器；提供后返回该元素内的文本，而不是 Readability 正文提取。',
