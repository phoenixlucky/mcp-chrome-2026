...
const zhDescriptions: Record<string, string> = {
  ...
  chrome_bookmark_add: '向 Chrome 添加书签。',
  chrome_bookmark_delete: '从 Chrome 删除书签。',
  ...
  chrome_request_element_selection: '请求用户在页面上选择一个元素。',
  ...
  chrome_get_page_text: '提取页面可读的正文、HTML 和文章元数据。',
  chrome_spa_fetch: '等待单页应用渲染和懒加载后，提取完整文本内容。',
  chrome_get_tab_url: '获取指定标签页的当前 URL、标题和图标。',
  chrome_get_scroll_state: '获取页面或滚动容器的位置与边界状态。',
  chrome_scroll: '滚动页面或指定滚动容器，支持像素、边界和元素定位。',
  chrome_wait: '等待 DOM 元素或 JavaScript 条件满足。',
  chrome_extract: '使用 CSS 选择器从页面提取结构化数据。',
...
const zhToolParameterDescriptions: Record<string, string> = {
  ...
  'chrome_cookie_get.storeId': '浏览器 Cookie 存储分区 ID。',
...
const zhParameterDescriptions: Record<string, string> = {
  ...
  expectedUrl: '操作完成后预期的 URL，用于验证结果。',
  name: '截图名称。',
