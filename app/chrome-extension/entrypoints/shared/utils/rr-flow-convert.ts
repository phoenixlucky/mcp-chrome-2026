...
// Both supported — kept empty for future truly-unsupported kinds
const UNSUPPORTED_NODE_KINDS = new Set<string>([]);
  ...
  return {
    flow: {
      ...
      variables: flow.variables?.map((variable) => ({
        ...
        sensitive: variable.sensitive,
        ...(variable.default !== undefined ? { default: variable.default } : {}),
        ...(variable.required ? { rules: { required: true } } : {}),
      })),
      nodes: flow.nodes.map((node) => ({
        id: node.id,
        type: node.kind as BuilderNode['type'],
        name: node.name,
...
export function isFlowV3(value: unknown): value is FlowV3 {
  ...
  return (
    ...
    typeof flow.entryNodeId === 'string' &&
...
export function extractFlowCandidates(parsed: unknown): unknown[] {
  ...
  if (
    ...
    Array.isArray((parsed as { flows?: unknown[] }).flows)
  )
    return (parsed as { flows: unknown[] }).flows.filter(isFlowV3);
  return [];
}
